    ...
    try {
        ...
        const res = await fetch(`${url.replace(/\/$/, '')}/v1/chat/completions`, {
            ...
            body: JSON.stringify({
                model: model,
                messages: [
                    { role: "system", content: "你是一个生成内容的助手。你的任务是严格按照用户提供的提示词生成文本或 HTML，输出内容必须只包含提示词要求的内容，不允许添加任何其他解释、总结、注释、额外文本或HTML标签以外的内容。 每条输出必须以提示词指定格式为准。如果提示词中指定了HTML结构或标签，请严格遵守。提示词可能包含多组 [主题] 标签。请分别生成每组对应内容，每组内容必须严格按照主题的 HTML 或文本格式输出。" },
...
genBtn.addEventListener('click', async () => {  
    ...
    const selectedWorldbooks = ['奇幻世界书A内容', '魔法世界书B内容'];  
